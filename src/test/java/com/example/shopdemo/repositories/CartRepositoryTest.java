...
class CartRepositoryTest {
    ...
    void setUp() {
        ...
        options = IntStream.range(0, product.optionSize()).mapToObj(product::option).toList();
        optionItems = IntStream.range(0, product.optionSize()).mapToObj(product::option)
                .map(option -> option.item(0)).toList();
    }
    ...
    void find() {
        ...
        assertThat(lineItem.optionIds()).hasSameElementsAs(options.stream().map(ProductOption::id).toList());
        ...
        IntStream.range(0, options.size()).forEach(index -> {
            ...
            assertThat(lineItem.productOptionItemId(optionId)).isEqualTo(optionItemId);
        });
    }
}