...
class OrderTest {
    ...
    void creation() {
        ...
        List<OrderLineItem> lineItems = List.of(
                new OrderLineItem(OrderLineItemId.generate(), product1, createOrderOptions(product1, new int[]{0, 0}), 1),
                new OrderLineItem(OrderLineItemId.generate(), product1, createOrderOptions(product1, new int[]{1, 1}), 2),
                new OrderLineItem(OrderLineItemId.generate(), product2, List.of(), 2));
        ...
        assertThat(order.totalPrice()).isEqualTo(new Money(128_000L + 128_000L * 2 + 123_000L * 2));
    }

    @Test
    @DisplayName("title - when lineitem size is 1")
    void titleWithOnlyOneLineItem() {
        Product product = Fixtures.product("맨투맨");

        List<OrderLineItem> lineItems = List.of(
                new OrderLineItem(OrderLineItemId.generate(), product, createOrderOptions(product, new int[]{0, 0}), 1));
    ...
    @Test
    @DisplayName("title - when lineitem size is 2")
    void titleWithTwoLineItems() {
        ...
        List<OrderLineItem> lineItems = List.of(
                new OrderLineItem(OrderLineItemId.generate(), product1, createOrderOptions(product1, new int[]{0, 0}), 1),
                new OrderLineItem(OrderLineItemId.generate(), product2, List.of(), 1));
